...
- 在 `custom`、`feature/*` 或基于 `custom` 创建的 Hotfix / 发布分支上工作前，必须先阅读并遵守 [CUSTOM_RULES.md](./CUSTOM_RULES.md)。
- 涉及分支职责、上游同步、Rebase、冲突处理、定制开发边界和发布基线时，以 `CUSTOM_RULES.md` 的专属规则为准；其他开发规范继续遵循本文档。
